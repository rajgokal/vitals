...
export default function ProviderCard({ provider }: ProviderCardProps) {
  return (
    <div className="rounded-xl border border-border bg-card p-4 hover:bg-card-hover transition-colors duration-150 animate-in">
      ...
      {(provider.practice || provider.facility) && <p className="text-xs text-muted mt-1">{provider.practice || provider.facility}</p>}
      {provider.address && <p className="text-xs text-muted mt-1">{provider.address}</p>}
